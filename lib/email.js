...
async function paymentConfirmedEmail(order) {
  const transport = getTransport();
  if (!transport) return;

  const total = order.payment ? order.payment.amount_total : order.items_total + order.shipping.amount;
  ...
  if (process.env.NOTIFY_EMAIL) {
    await transport.sendMail({
      ...
      text: `El pedido ${order.id} de ${order.customer.name} ya está pagado (${total.toFixed(2)} EUR). Puedes prepararlo para el envío.`,
    });
  }
  ...
  if (order.customer.email) {
    await transport.sendMail({
      from: process.env.SMTP_USER,
      to: order.customer.email,
      subject: `Pago recibido — pedido ${order.id} — Buddhabhumi`,
      text: `Hola ${order.customer.name},

Hemos recibido tu pago de ${total.toFixed(2)} EUR para el pedido ${order.id}. Lo preparamos y te avisaremos cuando salga hacia ${order.address.city}.
...
module.exports = { notifyShopOfNewOrder, confirmToCustomer, paymentConfirmedEmail };
